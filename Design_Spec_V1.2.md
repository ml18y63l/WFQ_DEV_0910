# wfq_tag_sort_engine — Design Specification V1.2
...
| 前一版本 | [Design_Spec_V1.1.md](Design_Spec_V1.1.md)，保留作为历史版本 |
| 本版本核心决定 | PTR_WIDTH=10、MEM_DEPTH=1024；派生 ADDR_WIDTH=10、COUNT_WIDTH=11；保持单份混合寄存器/SRAM Trie 和 II=5/6 |
| RTL 语言 | Verilog-2001（IEEE 1364-2001），源文件 .v、公共包含文件 .vh；不使用 SystemVerilog |
| 状态 | 规格及 Python 周期模型检查完成；尚无 RTL、RAM 宏仿真、综合或 STA 结果 |

本文取代 V1.1 作为后续实现基线。“必须”表示验收要求，“建议”表示在保持行为和时序契约前提下可选择的实现方式。保留完整功能与时序定义，并将指针存储位宽、物理地址位宽及容量计数位宽明确分离。
...
3. 默认共享 1024 个物理槽位、512 个 Flow ID，重复键分别占用槽位。
...
| V1.1 | [Design_Spec_V1.1.md](Design_Spec_V1.1.md) | 单副本、FAST5/PIPE6 排程及本版直接基础 |
...
| VL | [AMD UG901：Verilog 常量函数示例](https://docs.amd.com/r/en-US/ug901-vivado-synthesis/Example-of-Constant-Functions) | Verilog 常量函数的工具文档参考；不作为已完成编译/综合的证据 |

本文优先于 V1.1、V1.0 和分析文件。A1 中的上层同步读及 II=8 排程是中间方案，不是本文当前约束。论文转录存在缺失的图表，论文报告的频率不能作为本设计的 STA 结果。

### 1.4 相对 V1.1 的变化与可行性结论

| 事项 | V1.1 | V1.2 |
| :--- | :--- | :--- |
| 指针字段 PTR_WIDTH | 12 | 10，完整编码 0～1023 |
| 总节点容量 MEM_DEPTH | 4096 | 1024，仍为两 bank/全部流共享 |
| 参数关系 | MEM_DEPTH=2**PTR_WIDTH | 容量独立配置；ADDR_WIDTH=clog2(MEM_DEPTH)<=PTR_WIDTH |
| 节点 RAM 地址位宽 | 12 | 10，仅寻址 0～1023 |
| RC/容量计数 COUNT_WIDTH | 13 | 11，必须表示 1024 |
| TT/NEXT 条目宽度 | 13 | 11=1 valid+10 pointer |
| FREE 条目宽度 | 12 | 10；物理深度按 MEM_DEPTH 配置 |
| Trie、TT/RC 地址域 | 12-bit tag×两代 | 保持；不能随节点容量缩为 1024 个 tag |
| Trie 实现 | 上层单份组合读，L3 单份同步 1RW | 保持 |
| 发射/提交/修补/写回完成 | 完整 FAST5/PIPE6 排程 | 保持 |
| 逻辑状态位估计 | 475,680 bit≈58.07 KiB | 248,352 bit≈30.32 KiB |
| 越界保护 | 满地址空间节点池 | 默认 10-bit 指针无越界二进制编码；保留栈索引检查及较宽指针配置的 code=7 |

**默认采用 PTR_WIDTH=10、MEM_DEPTH=1024。** 10-bit 指针恰好覆盖全部 1024 个槽位，NULL 仍有独立 valid 位，因此 link 向量 总宽为 11 bit；计数器保持 11 bit 以表示满容量值 1024。

指针字段、节点 RAM 地址和容量计数分别按其语义定义；本版保留 PTR_WIDTH>=ADDR_WIDTH 的一般参数约束，但默认配置没有冗余指针位。

容量降为原来的四分之一，能够缓冲的描述符实例减少；512 个 Flow ID 不变，1024/512=2 只是平均容量比，不是每流配额或保证。系统需依据突发量和下游停顿评估是否足够；结构和排程可行不等于已证明业务流量不会满。
...
| `EPOCH_WIDTH` | 16 | 固定 |
| `EPOCH_BANKS` | 2 | 固定 |
| `PTR_WIDTH` | 10 | 4～16，且 >=ADDR_WIDTH；表示指针字段位宽 |
| `MEM_DEPTH` | 1024 | 独立配置；16～65536 范围内的 2 的幂；不再从 PTR_WIDTH 派生 |
| `ADDR_WIDTH` | 10 | 模块内部 localparam：`wfq_clog2(MEM_DEPTH)`，不得覆盖 |
| `FLOW_ID_WIDTH` | 9 | 8～12 |
| `COUNT_WIDTH` | 11 | 模块内部 localparam：`wfq_clog2(MEM_DEPTH+1)`，不得覆盖或改用 PTR_WIDTH+1 |
| `ISSUE_INTERVAL` | 5 | **仅允许 5 或 6，编译期配置，运行中不可切换** |
| `RSP_DEPTH` | 2 | 两项寄存器响应 FIFO |
| `MAX_INFLIGHT` | 2 | 已接受且尚未完成写回的事务数上限；仅等待响应消费的已完成事务不计入 |

`MEM_DEPTH` 是全部 bank/Flow ID 共享容量。不能把 tag 值域、每 bank metadata 深度和物理槽位数混为同一参数。

编译/展开前的配置检查必须确认：MEM_DEPTH 为上述范围内的 2 的幂、PTR_WIDTH>=ADDR_WIDTH、固定参数符合约束；COUNT_WIDTH 和 ADDR_WIDTH 由模块内部常量函数派生。默认 10/1024 合法；11/1024 是较宽指针配置；11/2048 合法且 COUNT_WIDTH=12；11/4096、10/2048、11/1000 不合法。本版不扩展到非 2 的幂容量。

ADDR_WIDTH 用于 DATA/NEXT/FREE 的物理索引；PTR_WIDTH 用于持久指针字段及指针比较；COUNT_WIDTH 用于 RC、queue_level、free_count、alloc_reserved 和 bank_count。默认前两者均为 10，计数宽度为 11，不能把容量计数缩到指针宽度。

`wfq_clog2(value)` 为 Verilog-2001 模块内常量函数，返回 ceil(log2(value))，完整定义见第 4.1 节。本文资源公式中的 clog2 表示同一数学函数，RTL 不依赖 `$clog2` 系统函数。
...
| `INSERT_WB_DONE_LATENCY`，WB | I+1 | 6 | 7 |
| `EXTRACT_COMMIT_LATENCY` | 1 | 1 | 1 |
| `EXTRACT_WB_DONE_LATENCY` | 2 | 2 | 2 |

FAST5/PIPE6 只是 I=5/6 的名称，不另设可产生矛盾组合的独立 mode 参数。非法组合须在编译/展开前被构建检查阻止；仿真还应在 time 0 检查参数，用 Verilog 的 $display/$finish 报错停止，由启动脚本将错误标记转换为失败退出状态。参数检查不能依赖 SystemVerilog 的 $fatal。

### 2.3 数据字段、地址和位序

字段使用定宽 reg/wire。link 打包为 {valid,ptr}，payload 打包为 {epoch,tag,flow_id}，高位在前：

~~~verilog
localparam integer LINK_WIDTH    = 1 + PTR_WIDTH;
localparam integer PAYLOAD_WIDTH = EPOCH_WIDTH + TAG_WIDTH + FLOW_ID_WIDTH;

reg  [LINK_WIDTH-1:0] link_q;
wire                 link_valid;
wire [PTR_WIDTH-1:0]  link_ptr;
assign link_valid = link_q[PTR_WIDTH];
assign link_ptr   = link_q[PTR_WIDTH-1:0];

reg  [PAYLOAD_WIDTH-1:0] payload_q;
wire [EPOCH_WIDTH-1:0]  payload_epoch;
wire [TAG_WIDTH-1:0]    payload_tag;
wire [FLOW_ID_WIDTH-1:0] payload_flow_id;
assign payload_epoch   = payload_q[PAYLOAD_WIDTH-1 -: EPOCH_WIDTH];
assign payload_tag     = payload_q[FLOW_ID_WIDTH +: TAG_WIDTH];
assign payload_flow_id = payload_q[FLOW_ID_WIDTH-1:0];
~~~

示例位于模块内部；指针、地址和计数分别声明为 [PTR_WIDTH-1:0]、[ADDR_WIDTH-1:0]、[COUNT_WIDTH-1:0]。文中 link.valid、TT.valid 等记法仅表示语义字段，RTL 用独立信号或向量切片，不使用结构体、类型别名或点号成员访问。

NULL 为 link.valid=0，ptr 此时不参与寻址。默认 link 向量 总宽 11=1 valid+10 pointer；全部 1024 个二进制指针编码均对应合法槽位 0～1023，地址 0 和 1023 均可分配。默认指针字段 不存在 bit 10，也不能编码 1024；不得保留一个可用地址作为 NULL 哨兵。

计数表示 0～1024，满容量为 `11'b10000000000`；10-bit 计数会回绕为 0。计数值 1024 合法，不代表指针值 1024 合法。
...
任何有效 head/tail、bank_head/bank_tail、TT/NEXT 指针，以及 FREE 读出的新分配指针，均须满足 `ptr<MEM_DEPTH`。默认 PTR_WIDTH=ADDR_WIDTH=10，节点指针直接作为同宽地址；对已定宽且无 X/Z 的 指针字段，此范围条件恒真，可由综合消去比较逻辑。

仅在 PTR_WIDTH>ADDR_WIDTH 的其他合法配置中，内部地址需要零扩展为 指针字段，解引用前必须检查其高位/范围。例如 11/1024 配置不能先丢弃 bit 10，使 1024/2047 别名为 0/1023。默认配置的较宽栈索引及临时算术同样须在转换为 10 位之前检查，不能依赖转换后的指针判断。

推荐使用显式无符号扩展实现一般参数下的范围判断：

~~~verilog
localparam integer ADDR_WIDTH = wfq_clog2(MEM_DEPTH);
localparam integer RANGE_WIDTH =
    ((PTR_WIDTH > COUNT_WIDTH) ? PTR_WIDTH : COUNT_WIDTH) + 1;
localparam [RANGE_WIDTH-1:0] MEM_LIMIT = MEM_DEPTH;

function ptr_in_range;
    input [PTR_WIDTH-1:0] p;
    reg [RANGE_WIDTH-1:0] wide_p;
    begin
        wide_p = {{(RANGE_WIDTH-PTR_WIDTH){1'b0}}, p};
        ptr_in_range = (wide_p < MEM_LIMIT);
    end
endfunction
~~~

上述 localparam 和函数放在模块内部，使用第 4.1 节的 wfq_clog2 定义。MEM_LIMIT 是显式无符号定宽常量；合法正整数 MEM_DEPTH 在赋值时转换到该宽度，不使用类型强制转换语法。示例不替代参数合法性检查。

只有指针有效且范围正确时才能使能节点 RAM 访问，并转换为 `p[ADDR_WIDTH-1:0]`；默认就是完整的 p[9:0]，不丢弃任何指针位。invalid link 必须屏蔽访问；FREE 有效栈项没有独立 valid 位，其可用性由 free_count 和分配生命周期保证。

一般配置下的范围检查须覆盖解引用前及最终节点写使能，不增加流水拍数；较宽指针越界使用第 13 节 code=7。默认配置不能仅凭范围检查发现“位翻转后指向另一个合法槽位”的错误，这类一致性由原有局部检查和验证不变量覆盖；不能强制保留不存在的高位检测或给 10-bit ptr 注入数值 1024 来要求 code=7。

节点地址转换不适用于 metadata：TT/RC 仍使用 13-bit `{bank,tag}`、L3 使用 9-bit `{bank,a,b}`；tag=4095 始终是合法标签。
...
下面采用 Verilog-2001 非 ANSI 端口声明，使模块内常量函数及派生位宽在端口声明前定义。示例是接口骨架，省略功能逻辑；输出采用 wire，由内部寄存器的连续赋值或子模块输出驱动。
...
module wfq_tag_sort_engine #(
    ...
    clk, rstn,
    insert_val, insert_ready, insert_epoch, insert_tag, insert_flow_id,
    insert_commit, insert_epoch_blocked,
    extract_req, extract_ready, extract_commit, extract_val, extract_out_ready,
    min_epoch_out, min_tag_out, min_tag_flow_id,
    init_done, empty, full, queue_level, busy, idle, fault, fault_code
);
    ...
    // Functional RTL is omitted from this interface example.
endmodule
~~~

COUNT_WIDTH 与 ADDR_WIDTH 均改为模块内部 localparam，不提供外部覆盖入口；集成方按 MEM_DEPTH 计算预期端口宽度。这样无需在参数端口列表中调用工具特有系统函数，也不会改变任何合法容量配置的数据宽度。ISSUE_INTERVAL 和提交周期保持 V1.1 契约。

默认 queue_level 输出从 V1.1 的 13 bit 变为 11 bit，集成方必须同步调整连接/寄存器映射；如外围保留更宽状态字段，应显式零扩展。tag、epoch、Flow ID 和 ready/valid 等其他顶层端口保持原宽度及语义。
...
| busy | 有已接受且尚未完成写回的事务，包括待完成旁路交接的事务 |
...
| wfq_commit_ctrl | 提前形成描述符主体、末级补齐、提交与写回完成 |
...
| TT | 8192×11 | 1 | 1 | 1R1W；1 valid+10-bit tail_ptr |
| RC | 8192×11 | 1 | 1 | 1R1W；0～1024 |
| DATA | 1024×37 | 1 | 1 | 1R1W；epoch+tag+flow_id，地址 10 bit |
| NEXT | 1024×11 | 1 | 1 | 1R1W；1 valid+10-bit next_ptr，地址 10 bit |
| FREE | 1024×10 | 1 | 1 | 1R1W；10-bit 空闲指针，地址 10 bit |

L2 的组合多读或前视是从一份寄存器状态扇出，不复制状态。不将 L1/L2 的输出强制再封装成 1 拍读接口，也不取消显式候选寄存器。

DATA/NEXT/FREE 必须按 MEM_DEPTH 分配逻辑条目；默认均为 1024 项，更宽指针配置也不自动增加节点数。FPGA/ASIC 宏若有深度粒度填充，超出 MEM_DEPTH 的物理部分不可分配，且需在物理资源报告单列。
...
NODE_ADDR_WIDTH = ADDR_WIDTH = clog2(MEM_DEPTH)
TT_ADDR_WIDTH = RC_ADDR_WIDTH = 1 + TAG_WIDTH = 13

L1/L2 状态寄存器 = 32 + 512 = 544 bit
L3 SRAM            = 8192 bit
TT                  = 8192*11 = 90112 bit
RC                  = 8192*11 = 90112 bit
DATA                = 1024*37 = 37888 bit
NEXT                = 1024*11 = 11264 bit
FREE                = 1024*10 = 10240 bit
同步存储数据位合计   = 247808 bit = 30.25 KiB
加 L1/L2 状态后合计  = 248352 bit ≈ 30.32 KiB
~~~

相对 V1.1 减少 227,328 bit，即 27.75 KiB，约减少 47.79% 的表/节点/上层状态位。容量减少 75%，总状态位不会同比减少 75%，因为 Trie 深度和 TT/RC 的条目数保持不变。

不包含事务/快照/输出寄存器、组合前视逻辑、ECC、宏粒度填充、BRAM 浪费及物理实现缓冲，不能等同于最终面积减少量。使用最小 10-bit 指针字段，无需为默认配置保存冗余高位。
...
~~~text
本沿新逻辑提交的值 > 最新的已提交但尚未完成写回的值 > 原 RAM q
~~~

同沿实际写总线需与此逻辑顺序一致；NEXT 延迟修补属于较早已提交事务的 pending overlay，不是新提交。清零、TT invalid、NEXT NULL 都必须能转发。

L3 不允许同沿读写，但 NEXT 必须支持，特别是旧事务 E_I 写 NEXT[p] 与新出队 E0 读取下一头 NEXT 的同址情况。读请求沿捕获旁路信息，不能在旧事务写回完成时丢失。

### 6.5 空闲地址栈

初始化仅写 i=0～MEM_DEPTH-1，FREE[i] 为地址 i 零扩展后的 指针字段、free_count=MEM_DEPTH。默认 FREE 只有 1024 个有效条目，第一项分配 n=1023，全部分配后包含地址 0。

插入接受时读旧 `FREE[free_count-1]`，free_count--、alloc_reserved++，E1 捕获 n、按配置检查范围并独占。默认 n 是 FREE 数据字中的完整 10-bit 指针，无需缩窄；栈索引由 11-bit free_count 计算，仍必须先检查再转换为 10-bit 地址。
...
栈地址算术先使用 COUNT_WIDTH 或更宽无符号临时值，检查目标索引 <MEM_DEPTH 后再转换为 ADDR_WIDTH；禁止先缩到节点地址宽度。free_count=0 不能减一读取；出队写栈必须满足沿前 free_count<MEM_DEPTH，不能把满计数 1024 截成地址 0。

FREE 写入的被释放 head.ptr 也须先检查范围。DATA/NEXT 释放后可保留旧比特，但无有效引用允许访问它们。
...
RC 等于该键的 live 节点数，全表 RC 总和等于 queue_level；默认必须能表示 1024。RC 不统计未提交预留和响应。RC 与容量计数均由 MEM_DEPTH 派生，不能继续用旧的 13-bit 固定宽度或 PTR_WIDTH+1 公式。
...
所有 live、FREE 有效栈项及预留节点的指针编码必须满足 0<=ptr<MEM_DEPTH，默认比较完整 ptr[9:0]。较宽指针配置中不能去掉高位后才判断范围或比较地址，否则会掩盖编码损坏。
...
**逻辑提交**在 E_C（插入）或 E1（出队）发生，表示队列状态原子可见，提交脉冲沿后保持一拍。

**写回完成（writeback complete）**在 E_WB（插入）或 E2（出队）发生，表示本事务全部 RAM 写入完成，已发出的相关读旁路信息也已交接，可以清除 pending 项并释放事务上下文。它与“最后一次 RAM 写入”分别计时：插入的最后写在 E_I，写回完成在 E_(I+1)；出队的 RAM 写在 E1，写回完成在 E2。即使某次操作无前驱修补，也按固定写回完成沿释放上下文。

**响应消费**另按 ready/valid，未消费响应不会延长写回阶段。验证检查器必须区分沿前采样和 NBA 后寄存输出。
...
| 写回完成 E_WB | E6 | E7 | 交接已发读的转发信息，释放旧上下文 |

E0 的 FREE/RC/L3 地址直接来自握手 payload、沿前 free_count，无需等待输入寄存器的下一拍输出。接受后其他组合查找只使用已捕获 payload。

E1 按配置检查分配 n 的范围；E_P 前用 TT q 或旧代尾得到 p，确认 p 有效且范围合法后才发 NEXT 读。默认指针与地址同宽，范围条件可常量折叠；一般配置的检查和地址转换也不能增加寄存阶段。
...
无前驱时 successor 来自已保存 h，重复键不写 Trie，A 命中不读备用叶；所有情况仍在 E_C 提交并在 E_WB 写回完成。
...
| E2 | 写回完成；FIFO 原空且下游 ready 时可在本沿消费响应 |
| E_I | 最早接受下一事务 |

出队无需遍历 Trie。E0～E1 包含 RAM q、RC 递减、并行位图判空/清位、下一头 epoch 检查及提交建立时间；PIPE6 不会自动放宽该路径。

E0 解引用缓存的 next 之前，及 E1 更新下一头缓存/释放地址之前，按参数配置检查指针。默认完整 10-bit 指针的范围条件恒真；较宽指针配置出现范围错误时禁止相关 RAM 访问/提交并置 code=7。默认配置仍检查 FREE 写栈索引边界，不能将 11-bit 满计数 1024 截成地址 0 后继续出队。
...
| T0 写回完成 / T1 提交 | 6 | 7 | 必须支持同沿释放旧上下文并接收新提交描述符 |
| T1 写回完成 | 7 | 8 | 响应仍可等待消费 |
| T2 最早接受 | 10 | 12 | 可安全重分配先前释放地址 |

若 T1 是插入，则在全局 2I-1 提交、2I 修补、2I+1 写回完成。前一事务已在下一次接受前提交；不允许两个未提交的插入同时在 Trie 中搜索。
...
V1.2 不改变上述理论操作率；缩小容量本身不增加发射周期。可缓冲槽位变少会使突发或持续失衡流量更早满，业务有效接受率可能降低。更小 RAM 是否提高 f_max 需重新综合/STA，新增范围检查也必须计入单周期路径，不能据存储位数直接宣称性能提高。
...
旧插入在 E_I 写 NEXT[p] 后，其 overlay 保留到 E_(I+1)；E_I 发出的同址读必须已捕获 bypass_data 并传给消费阶段。不能在写回完成沿清 valid，导致尚待本沿使用的 q 失去旁路。
...
下次可能分配该地址最早在 t+2I，此时 T0 已于 t+I+1 写回完成，且 t+2I>t+I+1。因此旧 NEXT 写不能覆盖新分配节点；不需要靠额外代数标签规避地址 ABA。

最大存活时长为插入 I+1 拍，小于两次发射间隔 2I，所以两个上下文足够。全局 t+I+1 可能同时完成 T0 写回并提交 T1 出队，控制器必须支持该同时事件；响应 FIFO 生命周期不占事务上下文。

上述证明同时适用于 I=5/6，依赖本文全部提交/修补沿和准入规则，不适用于独立改小 I 或延后写回。

证明不依赖容量恰为 4096，也不依赖 PTR_WIDTH 等于实际地址位宽；1024 槽下，只要 FREE 只提供合法地址、容量不预支、指针范围先检查且提交排程保持，释放/重分配先后关系相同。
...
| i<MEM_DEPTH | FREE[i] 写入零扩展到 PTR_WIDTH 的有效地址 i；默认只写 i=0～1023 |
...
节点池变为 1024 后，TT/RC 仍各有 8192 项，因此初始化时长保持 V1.1。扫描计数器须由 INIT_WRITES 派生，并能表达实现所用 guard 状态/边界；不能改用默认 11-bit COUNT_WIDTH 或 10-bit ADDR_WIDTH 扫描全部 metadata。
...
| 7 | 较宽指针配置中节点指针或有效 FREE 栈项编码越界：ptr>=MEM_DEPTH；默认 10/1024 无此类二进制编码 |
| 8～15 | 保留 |

code=3 的 successor 判定必须结合计数/边界：插入在 global tail 后得到 NULL 是合法情况。对未被选中的 speculative 候选不解引用 TT/NEXT。

code=7 保留给 PTR_WIDTH>ADDR_WIDTH 的合法参数配置，只检查有效 link 或必须有效的 FREE 条目；NULL 的 ptr 不参与检查/寻址。关键检测点包括 E1 的 FREE q、E_P 的 TT q/跨代尾、出队 E0 的缓存地址及 E1 的下一头 next，和最终节点写地址。默认 10-bit 指针在二值硬件中不会仅因范围产生 code=7；其范围逻辑可以消去。FREE 栈索引仍由计数边界检查保护，索引/容量错误按 code=5 处理，必须在缩窄前阻止读写。
...
| 1R1W wrapper | 同沿同址/异址、RAM 不同 read-during-write 模式、NULL/0 转发、写回完成与读消费同沿 |
| RC/TT | 0/1/2/MEM_DEPTH 边界、同值尾、最后键删除 |
| FREE | 全容量分配释放、地址 0/最大值、重分配及容量预留 |
| pointer/address guard | 默认 10-bit 的全部 1024 种编码均有效；11/1024 扩展配置另测越界不别名；默认较宽栈索引转换前检查；invalid link 不解引用 |
| commit/context | 主体提前寄存、末级直接提交、两上下文、上下文释放/新描述符入队 同沿、pending 生命周期 |
...
| F04 | 默认 1024 个同键填满，尝试第 1025 项后再排空 | RC=1024 可表示、满时不接受额外插入，最终全部计数/marker 归零 |
...
| F23 | E_C/E_I/E_WB 邻近复位 | 无幽灵提交，重新初始化 |
...
| F31 | 默认 PTR_WIDTH=10/MEM_DEPTH=1024 的全地址分配与再分配 | 完整覆盖 0～1023，指针与地址均为 10 bit，节点数组仅 1024 项 |
| F32 | 默认配置检查栈索引边界；11/1024 扩展配置注入指针 1024、1535、2047 | 默认防止计数缩窄别名；扩展配置检查 code=7、访问抑制和复位恢复 |
| F33 | 参数正/负例及三个不同位宽的检查 | 10/1024、11/1024、11/2048 合法；11/4096、10/2048、11/1000 不合法 |
| F34 | 默认容量跨两 bank 共用，tag=1024/2047/4095 | 总容量 1024，标签全值域保留、metadata 地址不按节点地址截断 |

F24 用 Verilog testbench 的层次化读写，或仿真器支持的 force/release 注入实际被读取的状态；若 overlay 会遮蔽 RAM 注入，应明确注入位置和生效周期。正常功能接口不新增 fault_inject 端口。测试至少包括提交沿前可见错误和同沿多错误优先级。

F32 的 code=7 注入必须在 PTR_WIDTH=11/MEM_DEPTH=1024 扩展配置运行；不能向默认 10-bit 字段赋值 1024 后把被截断的 0 当成越界注入。扩展配置的 invalid link 即使 ptr 高位为 1，也应保持不访问且不触发 code=7。默认配置另检查 11-bit 栈索引/计数的满空边界，非法索引按 code=5 禁止转换后访问。两者均监测 RAM enable，证明无别名污染；Python 范围函数检查不替代 RTL 故障控制验收。
...
小容量（MEM_DEPTH=16/32）增加满空转换和有界形式检查；默认容量完整执行 F04。参数验证必须显式指定 PTR_WIDTH/MEM_DEPTH 对，覆盖 4/16、5/16、10/1024、11/1024、11/2048、12/4096、16/65536，以及 F33 的非法组合；FLOW_ID_WIDTH=8/9/12 至少 elaboration。检查 COUNT_WIDTH 始终由实际容量派生，最大容量初始化使用派生 INIT_WRITES。

### 14.5 关键检查与不变量

以下“断言”表示必须成立的行为要求，基线使用 Verilog testbench 中的过程检查器实现：显式保存握手与预期周期，用 if/case、!== 等比较判断错误，通过 $display/$finish 及启动脚本报告失败。复杂全量队列对比可使用现有 Python 参考模型；不要求专用断言语言或结构化验证框架。
...
3. 提交时正确更新资源预留；写回完成时全部物理写和读旁路交接均结束，释放唯一上下文；在途上下文最多两个。
...
13. 每个 q 只供其 read_valid/事务/地址使用；E_I 读的旁路不可因 E_WB 写回完成失效。
14. trie_exact 等于该事务保存的 old_RC!=0；更新基于精确路径快照。
15. E_C 之前无本事务逻辑链路/metadata 修改；E_C 所有逻辑变更原子可见。
16. 无 fault 时 E_I 前驱修补及 E_WB 写回完成固定，不以热点或分支结果延后。
17. live/预留/FREE 有效栈项、有效 TT/NEXT/头尾指针均在 [0,MEM_DEPTH)；默认完整 10-bit 指针均可编码有效地址，较宽配置在检查前不丢弃高位。
18. DATA/NEXT/FREE 访问 enable 为 1 时实际地址小于 MEM_DEPTH，且来源有效指针或栈索引已经完整范围验证；metadata 地址不复用 ADDR_WIDTH。
19. 默认满容量计数为 1024，queue_level/RC/free_count 不截断为 0；满队列不接受第 1025 项，所有 bank 合计容量不超过 1024。
20. 较宽指针配置的 code=7 路径不发出非法地址访问或提交/释放；默认配置检查栈索引 code=5 的访问抑制；invalid link 不触发解引用。

全量状态性质放在 testbench/参考模型，不要求变成综合扫描硬件。检查器按实际采样/NBA 编写：可以在上升沿记录握手，在该周期下降沿检查稳定后的寄存输出和脉冲；不能把 #0 当作 NBA 完成，也不能把沿后脉冲解释成下一拍才提交。

### 14.6 完成标准

RTL 功能交付须完成 Verilog-2001 模式下的编译/lint、参数检查、单元与顶层仿真、上述定向/随机测试和无未解释检查失败。RTL 清单只包含 .v/.vh，不允许通过启用 SystemVerilog 模式绕过语法错误；测试启动脚本对参数或过程检查失败返回非零状态。综合后确认存储结构、端口、寄存器与流水延迟。频率/吞吐交付另须实际宏或器件模型、约束及 STA 证据。
...
**后续功能 RTL、RAM wrapper 和模块接口必须使用可综合 Verilog-2001（IEEE 1364-2001）**，文件后缀 .v，公共包含文件 .vh；顶层为 wfq_tag_sort_engine.v。不得使用 SystemVerilog 的 logic、typedef/struct/enum、package/interface、always_comb/always_ff、类型转换、非定长数组/队列或 unpacked array 端口。

组合逻辑使用 assign 或 always @(*)，给全默认值并禁止 latch；时序块使用 always @(posedge clk ...) 和 nonblocking 赋值。使用 reg/wire、parameter integer/localparam integer、模块内函数、generate 及定宽拼接/切片；循环索引在块的合法声明区定义，不在 for 初始化中声明类型。所有写使能保持原来的同步沿契约。

容量/地址计算采用本地 wfq_clog2 常量函数；参数合法性按第 2.1～2.2 节进行构建和 time 0 检查。mask/计数/指针运算显式定宽、invalid link 不寻址。reg 声明表示过程赋值变量，不自动等同于触发器，最终电路取决于 always 块和赋值方式。

基线 testbench 和过程检查器也使用 Verilog，软件参考/激励允许使用 Python；功能 RTL 编译不依赖 SystemVerilog 验证语法。本文代码片段用于定义接口、位序和函数契约，不代表已经交付功能 RTL。

DATA/NEXT/FREE 数组范围为 [0:MEM_DEPTH-1]、地址口为 ADDR_WIDTH；存储的指针字段为 PTR_WIDTH。默认 link 向量 使用 10-bit 指针及独立 valid，总宽 11。一般参数实现禁止用 [0:(1<<PTR_WIDTH)-1] 代替 MEM_DEPTH 分配节点数组，禁止用 PTR_WIDTH 推导容量计数宽度或 metadata 地址宽度。

L1/L2 使用 `reg [15:0] l1_regs [0:1]`、`reg [15:0] l2_regs [0:31]` 或等价寄存器结构；L2 以 {bank,a} 将 bank/节点索引展开为一维。跨模块传递独立标量/向量端口，不直接传递存储数组。正常更新是时钟沿写指定字；读端是组合选择。不能包进强制 1 拍读的 RAM wrapper，也不能为了“统一接口”额外寄存其读输出。PIPE6 的 aC/候选寄存属于明确的算法流水边界。
...
| 两者 | 指针有效性/访问门控 → 节点 RAM 地址或提交；默认已定宽指针的范围比较可消去，较宽配置需检查高位 |
...
| 1024 容量、10-bit 指针、11-bit 计数 | 1.4、2.1～2.4、6.5 | F31、F33～F34、有限位宽周期模型 |
| 越界编码不别名访问 | 2.4、10、13 | F32、指针/地址断言 |
| Verilog-2001 RTL 与验证基线 | 2.3～2.4、4.1、14.5～14.6、15.1 | Verilog 模式编译/lint、参数及过程检查 |
| 写回完成与逻辑提交分离 | 2.2、10.1～10.4、11.3 | 固定周期检查、F22～F23、F26 |
...
默认 PTR_WIDTH=10、MEM_DEPTH=1024，节点地址和计数分别派生为 10/11 bit；允许第 2 节范围内的其他编译期合法参数组合。冻结 valid 与 ptr 分离、栈索引及较宽指针在缩窄前检查、metadata 深度由 tag/epoch 决定。

冻结 Verilog-2001 语言及 .v/.vh 文件约定；统一使用“写回完成”表示物理写入和读旁路交接结束、上下文可释放，常量命名为 INSERT_WB_DONE_LATENCY/EXTRACT_WB_DONE_LATENCY，插入边沿记为 E_WB。命名调整不改变发射间隔、逻辑提交沿、最后写沿或响应时序。

### 16.3 需要后续版本的变化

超过两代共存、tag 宽度/Trie 分层变化、运行中切换 I/容量、I 不为 5/6、非 2 的幂容量、接受同拍双操作、提交/写回完成沿改变、L3 多端口或倍频、任意删除/peek/packet ID 接口都需要重新设计相应协议和证明。把冗余指针位改作 valid 或 NULL 哨兵也属于编码契约变化。
...
| V1.2 | 默认 PTR_WIDTH=10/MEM_DEPTH=1024、COUNT_WIDTH=11；更新资源/容量验证；统一 Verilog-2001 和“写回完成”术语，保留 V1.1 的实际排程 |
...
on E_P:
    validate p range, then read NEXT[p] if p.valid
    latch all successor-independent commit fields
on E_C:
    complete successor-dependent fields from returned NEXT / saved head
    validate all effective pointers and target node addresses before access
    if all checks pass:
...
on E_WB:
    finish writeback and release context after bypass handoff

on extract_accept(E0):
    reserve response slot; save current head_cache
    validate cached pointers; read old-head RC/L3 and next-head DATA/NEXT
...
on E2:
    finish writeback and release context; response remains until downstream handshake
~~~
...
4. 只修改 I，却保留旧 E7/E8/E9 提交/修补/写回完成。
...
9. 默认 10-bit RC/free_count 无法表示 1024；COUNT_WIDTH 错误地从 PTR_WIDTH+1 推导；count 截断后再检查边界。
10. TT 指向组首、同值第一次出队便清 TT/marker、遗漏地址中的 bank。
11. 修补 NEXT RAM 却忘记 head_cache.next，或只有缓存修补而漏掉 B 节点预取 RAW。
12. E_I 写完即清 overlay，E_WB 消费的同步读丢失旁路；NULL 写不参与转发。
...
19. 默认配置仍保留 11-bit 指针或遗漏 link 向量的 valid 位；正确 link 向量为 1+10=11 bit。
20. 把 11-bit 栈索引或扩展配置的较宽指针先截成 10 位再检查，造成低位别名；给默认 ptr 访问不存在的 bit 10。
21. 缩小节点容量时，把 TT/RC 深度、12-bit tag 值域和初始化扫描错误地一起缩到 1024。

## 附录 C：本版本已执行的规格模型检查

本版新增 [model/wfq_v12_check.py](model/wfq_v12_check.py)，使用 Python 3 标准库；复用 [model/wfq_v11_check.py](model/wfq_v11_check.py) 的周期引擎和端口排程检查，增加按实际深度的地址检查、有限位宽/值域约束和 1024 容量测试。稳定队列参考仍使用未截断 epoch 和插入序号；模型验证用的序号不属于 RTL payload。

在仓库根目录运行：

~~~powershell
py -3 -B model/wfq_v12_check.py
~~~

10-bit 指针配置最近一次已执行的模型检查结果：

| 检查 | 已执行范围与结果 |
| :--- | :--- |
| 参数正/负例 | 7 组合法、8 组非法组合按本版范围/位宽契约检查通过 |
| 默认指针编码穷举 | 10 位的全部 1024 种编码均合法；完整映射到 0～1023 |
| 较宽指针配置穷举 | 11/1024 配置的 2048 种编码：1024 个合法、1024 个越界拒绝；未产生截断别名 |
| 最坏端口排程 | 两配置的全部 2/3/4 操作类型序列，共 56 组通过 |
| FAST5 默认满容量 | 1024 个 epoch=65535/tag=4095 同键填满/排空，2048 次操作；地址 0～1023 全覆盖 |
| PIPE6 默认满容量 | 同上，2048 次操作通过 |
| FAST5 两代容量/重用 | 两代共享 1024 槽、第三代阻塞、旧 bank 清空后重用，共 3072 次操作通过 |
| PIPE6 两代容量/重用 | 同上，3072 次操作通过 |
| FAST5 混合周期模型 | 11,024 次混合/排空操作；46 次同沿 NEXT RAW；全部响应一致 |
| PIPE6 混合周期模型 | 11,028 次混合/排空操作；39 次同沿 NEXT RAW；全部响应一致 |

V1.1 已记录的 1,163,264 次前驱/配置比较是历史算法检查；本次不把它重复计入新配置的已执行数量。Trie 几何结构未变，本次混合测试另外包含 tag=0/1023/1024/2047/4095，证明节点地址缩窄不限制标签范围。

周期模型区分物理 RAM、同步读返回、逻辑提交、pending NEXT 修补、读沿旁路、分配/释放、头缓存、两代状态、响应预留和消费；本次在所有正常访问及写值上加入节点深度、指针及 RC 范围断言。默认容量和两种排程的以上用例全部通过。

这些结果支持“默认 10-bit 指针/1024 槽在 5/6 拍排程下可行”。较宽指针配置的非法编码测试只验证范围转换契约，并未实现完整硬件 fault 控制、F32 的访问抑制注错仿真或 RTL elaboration。模型也不替代初始化电气时序、全部参数组合、第 14 节完整验收、RAM 宏仿真、综合、STA 和功耗评估；目标 MHz 仍非实测结果。

本次 Verilog-2001 与写回完成术语修订仅涉及规格/仓库说明，以上周期模型及其排程未改动，检查数量沿用该次运行。已核对代码示例的语言写法、接口信号、位宽及周期表；当前没有实际 Verilog 编译或综合结果。
