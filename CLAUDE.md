...
**Current state: Stage 1 RTL and unit/integrated-storage simulations are complete.** See `RTL_Implementation_Plan.md` and `verification/RTL_Stage1_Report.md`. Six modules implement the matcher, reset synchronizer, single-copy upper Trie registers, synchronous 1RW/1R1W RAM wrappers and initialization controller. The full `wfq_tag_sort_engine` top and transaction controllers are not implemented yet. Run `py -3 -B scripts/run_stage1.py` for strict Verilog-2001 RTL regression, or `py -3 -B model/wfq_v12_check.py` for the separate capacity/pointer/cycle specification model. There is no synthesis or STA result. Do not claim RTL performance results — target frequencies (250–300 MHz ASIC / 125–150 MHz FPGA) are unverified goals.
