"""FAST5 stage 2 Verilog-2001 unit, integration, schedule and fault regression.

Run from any directory: py -3 -B scripts/run_stage2.py [--quick]
The quick run skips the all-tag query sweep and reduces the mixed operation
stream. Uses the same local Icarus setup as run_stage1.py; never downloads tools.
"""
import argparse
import hashlib
import json
...
def main():
    ...
    cases = [("tb_wfq_metadata_update", {"PTR_WIDTH": ptr, "MEM_DEPTH": depth})
             for ptr, depth in [(4, 16), (10, 1024), (11, 1024), (16, 65536)]]
    cases += [
        ("tb_wfq_key_metadata", {"PTR_WIDTH": 4, "MEM_DEPTH": 16,
                                "FULL_SWEEP": 0, "RANDOM_OPS": 200 if args.quick else 1000}),
        ("tb_wfq_key_metadata", {"PTR_WIDTH": 10, "MEM_DEPTH": 1024,
                                "FULL_SWEEP": 0 if args.quick else 1,
                                "RANDOM_OPS": 200 if args.quick else 2000}),
        ("tb_wfq_key_metadata", {"PTR_WIDTH": 11, "MEM_DEPTH": 1024,
                                "FULL_SWEEP": 0, "RANDOM_OPS": 200 if args.quick else 1000}),
        ("tb_wfq_metadata_faults", {"PTR_WIDTH": 10, "MEM_DEPTH": 1024}),
        ("tb_wfq_metadata_faults", {"PTR_WIDTH": 5, "MEM_DEPTH": 16}),
    ]
    ...
    invalid = [
        ({"ISSUE_INTERVAL": 4}, "wfq_error_stage2_requires_issue_interval_5"),
        ({"ISSUE_INTERVAL": 6}, "wfq_error_stage2_requires_issue_interval_5"),
    ...
    for idx, (params, diagnostic) in enumerate(invalid):
        result, _ = compile_case("tb_wfq_key_metadata", params, f"invalid_{idx}")
    ...
    summary["status"] = "PASS"
    (BUILD / "results.json").write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
    print(f"PASS Stage 2 FAST5: {len(cases)} simulations; {len(invalid)} rejected configurations", flush=True)
