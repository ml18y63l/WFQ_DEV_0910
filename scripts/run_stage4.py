"""FAST5 stage 4 full-engine Verilog-2001 regression.

Run from any directory: py -3 -B scripts/run_stage4.py [--quick]
The quick run reduces random traffic to 1000 cycles per configuration. Directed
full-capacity, bank, RAW, fault and reset checks are unchanged. Uses the local
...
def main():
    ...
    cases = [("tb_wfq_tag_sort_engine", {
                "PTR_WIDTH": ptr, "MEM_DEPTH": depth, "FLOW_ID_WIDTH": flow,
                "SEED": seed, "RANDOM_CYCLES": 1000 if args.quick else 15000})
             for ptr, depth, flow, seed in [(4, 16, 8, 12345), (10, 1024, 9, 12345),
                                            (10, 1024, 9, 5381), (11, 1024, 12, 67891)]]
    cases += [("tb_wfq_engine_faults", {"PTR_WIDTH": ptr, "MEM_DEPTH": depth})
              for ptr, depth in [(10, 1024), (11, 1024)]]
    ...
    for top, params in cases:
        ...
        result, elapsed = execute([vvp, "-N", str(output)], env, BUILD / (label + ".run.log"), 600)
    ...
    invalid = [
        ({"ISSUE_INTERVAL": 4}, "wfq_error_stage4_requires_issue_interval_5"),
        ({"ISSUE_INTERVAL": 6}, "wfq_error_stage4_requires_issue_interval_5"),
        ...
        ({"TAG_WIDTH": 11}, "wfq_error_fixed_tag_literal_epoch_widths"),
        ({"EPOCH_WIDTH": 15}, "wfq_error_fixed_tag_literal_epoch_widths"),
        ({"LITERAL_WIDTH": 3}, "wfq_error_fixed_tag_literal_epoch_widths"),
        ({"PTR_WIDTH": 11, "MEM_DEPTH": 4096}, "wfq_error_invalid_node_capacity_or_pointer_width"),
        ({"PTR_WIDTH": 10, "MEM_DEPTH": 2048}, "wfq_error_invalid_node_capacity_or_pointer_width"),
    ]
    for idx, (params, diagnostic) in enumerate(invalid):
        result, _ = compile_case("tb_wfq_tag_sort_engine", params, f"invalid_{idx}")
    ...
    summary["status"] = "PASS"
    (BUILD / "results.json").write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
    print(f"PASS Stage 4 FAST5: {len(cases)} simulations; {len(invalid)} rejected configurations", flush=True)
